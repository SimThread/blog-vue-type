...
exports.customTime = (item) => {
  ...
  const historyTime = window.parseInt(nowTime) - window.parseInt(publishTime);
  let descTime;
  if (historyTime >= yearTime) {
    // 按年算
    descTime = `${window.parseInt(historyTime/yearTime)}年前`;
  } else if (historyTime< yearTime && historyTime >= monthTime) {
    // 按月算
    descTime = `${window.parseInt(historyTime/monthTime)}月前`;
  } else if (historyTime< monthTime && historyTime>= dayTime) {
    // 按天算
    descTime = `${window.parseInt(historyTime/dayTime)}天前`;
  } else if (historyTime< dayTime && historyTime>= hourTime) {
    // 按小时算
    descTime = `${window.parseInt(historyTime/hourTime)}小时前`;
  } else if (historyTime< hourTime && historyTime>= minuteTime) {
    // 按分钟算
    descTime = `${window.parseInt(historyTime/minuteTime)}分钟前`;
  } else {
    descTime = '刚刚';
  }
  return descTime;
